...
class ToolSandbox:
    ...
    def _get_tool(self, tool_name: str) -> Any:
        ...
        Tries both tools registry and agents tool registry.
        """
        # Try tools registry first (preferred)
        try:
            from livingai.tools.registry import ToolRegistry as ToolsRegistry
        ...
        # Fallback to agents tool registry
        try:
            from livingai.agents.tool_registry import ToolRegistry as AgentsToolRegistry
            agents_registry = AgentsToolRegistry()
            tool_def = agents_registry.get_tool(tool_name)
            if tool_def:
                # Return the tool definition or create an instance
                return tool_def
        except ImportError:
            pass
        
        # If no registry is available, return None
        return None
